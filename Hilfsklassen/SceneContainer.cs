    ...
    static class SceneContainer
    {
        ...
        public static Scene scene11(int resX, int resY) // Floor again :)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                {
                    height = r.Next(1, maxHeight*10);
                    theEntities[amount*i+j] = new Cuboid(new Vector(i * (size / amount), j * (size / amount), (baseSize / 2) + (height / 20)) - new Vector((size - (size / amount)) / 2, (size - (size / amount)) / 2, -offsetUpDown), new Vector(1, 0, 0), new Vector(0, 0, 1), new double[3] { size / (amount) - s, size / (amount) - s, (height / 10) + baseSize }, floorMaterial);
                }
            }
